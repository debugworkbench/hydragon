...
export class DebugConfigManager {
  ...
  @mobx.observable
  debugConfigs: mobx.IObservableArray<IDebugConfig> = <any>[];

  constructor(private _loader: IDebugConfigLoader) {
  }
  ...
  async save(debugConfig: IDebugConfig): Promise<void> {
    ...
    await this._loader.write(configs);
    this.debugConfigs.replace(configs);
  }
  ...
  async load(): Promise<void> {
    ...
    if (this._loader.canRead()) {
      const configs = await this._loader.read();
      this.debugConfigs.replace(configs);
    }
  }
}
